// Build the dbm module into two self-contained runtime artifacts under dist/:
//   src/index.tsx -> dist/modules/dbm/client.js   (ESM, react external)
//   server.ts     -> dist/modules/dbm/server.js   (ESM, --target bun)
//
// A single-module distillation of the tabterm host's scripts/build-modules.ts.
// Like the timer module, dbm imports no CSS and does no dynamic import(), so
...
const OUT = join(REPO, "dist", "modules", "dbm");
...
function cssPrelude(css: string): string {
  return `(function(){try{if(typeof document==="undefined")return;` +
    `if(document.getElementById("tabterm-dbm-styles"))return;` +
    `var s=document.createElement("style");s.id="tabterm-dbm-styles";` +
...
console.log(`[build] dbm → ${join("dist", "modules", "dbm")}/{client.js,server.js}`);
